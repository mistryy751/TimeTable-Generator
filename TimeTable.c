//Code for generating time table for teachers based on different constraints:
#include <stdio.h>
#include <string.h>
#include <stdbool.h>
void print ();
...
printf
    ("Enter Total Working Days,Number of Lectures,Number of Cources,Lab Teachers,Number of Rooms:\n");
...
strcpy (fisub[i].fisub[j].fisub[k].fisub,
		   tmpsub[i].tmpsub[j].tmpsub[k].tmpsub);
...
if (((j == (lects / 2) - 1
	     && (ro[co[i].room].busy[j + 1].busy[k].busy == false))
	    && (ro[co[i].room].lunch[0].lunch[k].lunch == false))
	   || (j == (lects / 2)
	       && (ro[co[i].room].busy[j - 1].busy[k].busy == true)
...
strcpy (tmpsub[i].tmpsub[j].tmpsub[k].tmpsub, "\tLunch Hour");
...
strcpy (tmpsub[i].tmpsub[j].tmpsub[k].tmpsub, "\tBreak");
